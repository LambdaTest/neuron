...
func recursivelySet(val reflect.Value, prefix string) error {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		for _, tag := range tags {
			...
			case reflect.Map:
				if thisField.Type() == reflect.TypeOf(map[string]string{}) {
					mp := viper.GetStringMapString(key)
					thisField.Set(reflect.ValueOf(mp))
				}
...
func getTags(field *reflect.StructField) []string {
	...
	values := []string{}
	if tag != "" {
		for _, prefix := range []string{tagPrefix, "yaml", "json", "env"} {
